...
export async function authRouter (fastify, options) {
    fastify.post('/api/auth/registration', {schema: authSchema.registration }, registrationHandler)

    fastify.get('/api/auth/registration', {schema: authSchema.registrationConfirm}, registrationConfirmationHandler)

    fastify.post('/api/auth/login', {schema: authSchema.login }, loginHandler)

    fastify.get('/api/auth/logout', {onRequest: fastify.authenticate }, logoutHandler)

    fastify.post('/api/auth/password-reset', {schema: authSchema.changePassword}, changePasswordHandler)

    fastify.get('/api/auth/password-reset', (req, rep) => {
        rep.sendFile('reset-password.html')
    })

    fastify.post('/api/auth/password-reset/confirm', {onRequest: fastify.authenticate, schema: authSchema.changePasswordApprove }, changePasswordApproveHandler)
}
