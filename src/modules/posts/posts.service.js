...
class PostsService {
  ...
  async patchPost(
      ...
      authorId,
      role,
      {status, addCategories, deleteCategories, content},
  ) {
    const post = await this.prisma.post.findUnique({
      where: {id},
    });

    if (!post) throw new createError('FST_DB', 'Not found', 404);
  ...
  async deleteLike(userId, postId) {
    ...
    if (post) return likesService.deleteLike(userId, postId, post.authorId);
    return '';
  }
}
