...
export async function categoriesRouter(fastify, options) {
  ...
  fastify.get(
      '/api/categories/:id',
      {schema: categoriesSchema.getCategory},
      getCategoryHandler,
  );
  fastify.get(
  ...
  fastify.post(
      '/api/categories',
      {schema: categoriesSchema.createCategory},
      createCategoryHandler,
  );

  fastify.patch(
      '/api/categories/:id',
      {schema: categoriesSchema.patchCategory},
...
async function getPostsHandler(req, rep) {
  return categoriesService.getPostsWithCategory({
    id: req.params.id,
    status: req?.user?.role === 'ADMIN' ? undefined : 'ACTIVE',
  });
}
async function updateCategoryHandler(req, res) {
  return categoriesService.patchCategory(req.params.id, {
